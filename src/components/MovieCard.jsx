    ...
    <>
      ...
      <div>
        <div className="card relative hover:scale-105 transition-all duration-300" >
          <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="w-full md:h-[350px] h-full shadow-custom object-cover rounded-md" />
          <div className="bg-black/[0.4] absolute top-0 right-0 h-full w-full z-10 rounded-md" onClick={() => navigate(`/movie-details/${movie.id}`)}></div>
          ...
          <div className="bg-[#455e94] py-1 px-2 absolute bottom-2 left-2 z-20 rounded text-gray-100">
            <h5 className="text-sm font-semibold">{getReleaseYear(movie.release_date)}</h5>
          </div>
        </div>
